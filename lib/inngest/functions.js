...
const getMonthlyStats = async (userId, month) => {
  ...
  return { 
    ...
    totalExpenses: Math.round(acc.totalExpenses * 100) / 100,
    net: Math.round((acc.totalIncome - acc.totalExpenses) * 100) / 100,
    byCategory: Object.fromEntries(
      Object.entries(acc.byCategory).map(([key, value]) => [key, Math.round(value * 100) / 100])
    ),
    transactionCount: transactions.length 
  };
};
